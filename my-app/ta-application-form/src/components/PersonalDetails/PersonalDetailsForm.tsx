...
const PersonalDetails: React.FC<PersonalDetailsProps> = ({ onNext }) => {
  ...
  const [dbsCheck, setDbsCheck] = useState({
    needNewDBS: false,
    haveDBSOnUpdateService: false,
  });
  ...
  const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setDbsCheck({
      ...
      haveDBSOnUpdateService:
        name === "haveDBSOnUpdateService" ? checked : false,
    });
  };
  ...
  return (
    <form>
      ...
      <PhoneInput
        className="py-4 border border-gray-300 pl-4 rounded-[4px]"
        international
        defaultCountry="GB"
        value={phoneNumber}
      ...
      <TextField
        label="Town of Birth"
        value={townOfBirth}
        onChange={(e) => setTownOfBirth(e.target.value)}
        fullWidth
        margin="normal"
      />
      <TextField
      ...
      <Typography variant="body2" gutterBottom>
        If Hanson Recruitment are completing a new DBS for you, it will be Child
        and Adult Workforce. Do you need a new DBS or do you have one (child
        workforce/ child & adult workforce) on the update service?
      </Typography>
      <FormControlLabel
        control={
          <Checkbox
      ...
      />
      <Typography variant="body2" gutterBottom>
        If you have a DBS on the update service, we will need a copy of the DBS
        sent to us, and the physical copy must be presented to the schools when
        you go in for work.
      </Typography>
